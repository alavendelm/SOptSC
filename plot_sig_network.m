...
%% plot cluster-cluster signaling network based on each ligand-receptor pair
for j = 1:No_LR
    P = Pidv{j};
    P(P<=threshold) = 0;
    ...
    Gh = plot(bg,'Marker','o','MarkerSize',25,'Layout','circle','NodeLabel',[],...
        'NodeColor',mycolor,'EdgeColor',[0.5,0.5,0.5],'LineWidth',...
        bg.Edges.LWidths,'ArrowSize',15);
...
%% plot cluster-cluster signaling network based on all ligand-receptor pairs
P = Pall;
P(P<=threshold) = 0;
P_cluster = zeros(No_cluster);
